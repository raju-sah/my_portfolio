...
@section('content')
    <div class="container-xxl">
        ...
        <x-breadcrumb listRoute="{{ route('admin.backgrounds.index') }}" model="Background" item="Create"></x-breadcrumb>

        <div class="card">
            <div class="card-body">

                <x-form.wrapper action="{{ route('admin.backgrounds.store') }}" method="POST" enctype="multipart/form-data">
                    <x-form.row>
                    ...
                    <x-form.row>
                        <x-form.input type="text" col="6" label="address" id="address" name="address"
                            value="{{ old('address') }}" />
                        <x-form.input type="text" col="6" label="map_url" id="map_url" name="map_url"
                            value="{{ old('map_url') }}" />
                    </x-form.row>
                    <x-form.input type="text" label="web_url" id="web_url" name="web_url"
                        value="{{ old('web_url') }}" />
...
@push('custom_js')
    {!! JsValidator::formRequest('App\Http\Requests\Admin\BackgroundRequest') !!}
